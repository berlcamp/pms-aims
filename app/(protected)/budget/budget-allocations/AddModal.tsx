...
const FormSchema = z.object({
  ...
  lasaId: z.string().min(1, "LASA is required"),
...
export const AddModal = ({
  ...
  // Get current fiscal year from form
  const currentFiscalYear = form.watch("fiscalYear");

  // Fetch LASA options filtered by fiscal year
  useEffect(() => {
    const fetchLasaOptions = async () => {
      if (!user?.division_id || !currentFiscalYear) return;
      try {
        ...
        // Clear selected LASA if it doesn't exist in the filtered options
        const currentLasaId = form.getValues("lasaId");
        if (currentLasaId && !data?.some((lasa) => lasa.id === currentLasaId)) {
          form.setValue("lasaId", "");
        }
      } catch (error) {
  ...
  }, [isOpen, user?.division_id, currentFiscalYear, form]);
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditMode
              ...
              : "Create a new budget allocation to break down budgets into specific allocations."}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        LASA <span className="text-red-500">*</span>
                      </FormLabel>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {lasaOptions.map((lasa) => (
                            <SelectItem key={lasa.id} value={String(lasa.id)}>
                              {lasa.project_title || "Untitled Project"} (FY{" "}
                              {lasa.fiscal_year})
                            </SelectItem>
                          ))}
                        </SelectContent>
