...
import { Permission, RootState } from "@/types";
...
export const PermissionsList = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (filter.keyword) {
        query = query.or(
          `code.ilike.%${filter.keyword}%,name.ilike.%${filter.keyword}%,resource.ilike.%${filter.keyword}%`
        );
      }
  ...
  const handleDelete = async () => {
    if (selectedItem) {
      ...
      const { data: rolePermissions, error: checkError } = await supabase
        ...
        .select("id")
        .eq("permission_id", selectedItem.id)
        .limit(1);

      if (checkError) {
        toast.error("Error checking permission assignments");
        return;
      }
    ...
    <div>
      <div className="app__title">
        <div className="app__title_actions">
          <PermissionsFilter filter={filter} setFilter={handleFilterChange} />
          <Button
            variant="green"
            onClick={() => {
              setSelectedItem(null);
      ...
      <div className="app__content">
        {loading ? (
          ...
          <div className="app__empty_state">
            ...
            <p className="app__empty_state_title">No permissions found</p>
              ...
              <table className="app__table">
                <thead className="app__table_thead">
                  <tr>
                    ...
                    <th className="app__table_th">Module</th>
                    <th className="app__table_th">Resource</th>
                    <th className="app__table_th">Action</th>
                ...
                <tbody className="app__table_tbody">
                  {list.map((item: ItemType) => (
                    <tr key={item.id} className="app__table_tr">
                      ...
                      <td className="app__table_td">
                        <span className="app__badge">{item.module}</span>
                      </td>
                      <td className="app__table_td">
                        <span className="text-sm">{item.resource}</span>
                      </td>
                      <td className="app__table_td">
                        <span className="app__badge">{item.action}</span>
                      </td>
